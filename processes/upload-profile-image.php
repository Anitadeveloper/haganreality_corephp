...
if(isset($_FILES['file']['name'])){
	...
	while($generated_name === false){
		...
		$rand_name_file = $root.'images/user-profile/' . $rand_name . '.' . $ext;
		$num_rows = 1;
		if($stmt = $conn->prepare("SELECT * FROM admin_user_images WHERE profile_src=?")){
	...
	if($sql = $conn->prepare("SELECT profile_src FROM admin_user_images WHERE user_id=?")){
		...
		if($src != $_SERVER['DOCUMENT_ROOT'].'/images/user-profile/default.jpg'){
			unlink($src);
		}
		$sql->close();
	}
	...
	//save new image
	$target = $_SERVER['DOCUMENT_ROOT'] . '/images/user-profile/' . $newname;
	$result = copy($_FILES['file']['tmp_name'], $target);
	
	$file_name = $root . 'images/user-profile/' . $newname;
	//store name in database
	if($sql = $conn->prepare("UPDATE admin_user_images SET profile_src=? WHERE user_id=?")){
...
if($sql = $conn->prepare("UPDATE admin_image_positioning SET center_x=?, center_y=?, scale=? WHERE image_type=0 AND user_id=?")){
