...
if($sql = $conn->prepare("SELECT profile_src, cover_src FROM admin_user_images WHERE user_id=?")){
	$sql->bind_param('i', $user_id);
	//execute query
	$sql->execute();
	$sql->bind_result($profile_src, $cover_src);
	$sql->fetch();
	$return[1]->src = $profile_src;
	$return[2]->src = $cover_src;
	$sql->close();
}
...
if($sql = $conn->prepare("SELECT center_x, center_y, scale FROM admin_image_positioning WHERE user_id=? AND image_type=?")){
	...
	$sql->bind_result($center_x, $center_y, $scale);
	$sql->fetch();
	$return[2]->center_x = $center_x;
	$return[2]->center_y = $center_y;
